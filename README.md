...
- 付费关键帧需要 `TOKENDANCE_API_KEY`；付费视频需要 `NANYAO_API_KEY`
...
现有 `AI全媒体运营/`、`成品/`、`素材/` 是本机历史媒体挂载目录，整目录
Git 忽略，不属于仓库目录契约；保留原位仅为避免未经确认搬动用户素材。
...
- 当前默认路由是 TokenDance 关键帧 + nanyao Grok Fast 视频；Fast 每次默认生成最长的 10 秒，再按分镜时长裁切。模型、协议、分辨率和价格仍可能变化，付费前必须核对实时控制台。
